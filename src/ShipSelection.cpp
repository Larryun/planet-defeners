...
ShipSelection::ShipSelection(const sf::Texture& texture, const sf::Vector2f& pos) : Menu(0.f, 0.f)
{
    shipTexture = texture;
    position = pos;
...
void ShipSelection::setPlayerVector(sf::Vector2f shipsize){
    for(auto i = 0U; i < MAX_NUMBER_OF_ITEMS; i++){
        playerArray.push_back(new Player(shipTexture, shipRect[i], shipSize));
